...
export function resolveUploadMeta(content) {
  ...
  // the per-room upload dir. basename leaves "." and ".." intact, though, and
  // both resolve to a directory inside path.join — so fold them (and the empty
  // case) into the 'file' fallback, keeping every upload inside the room dir.
  const base = path.basename(content.filename || content.body || 'file');
  const filename = base === '' || base === '.' || base === '..' ? 'file' : base;
  const caption =
