...
// worktree name (or null) and the remaining positional tokens. The name is
// sanitised to alphanumeric + hyphens to keep branch/directory names safe.
export function extractWorktreeFlag(tokens) {
  let worktree = null;
  const rest = [];
  for (let i = 0; i < tokens.length; i++) {
    if (tokens[i] === '--worktree' && i + 1 < tokens.length) {
      worktree = tokens[++i].replace(/[^a-zA-Z0-9_-]/g, '-').slice(0, 64);
    } else {
      rest.push(tokens[i]);
    }
  }
